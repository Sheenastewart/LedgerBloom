    ...
    try {
      ...
      setProfileErrors(
        error instanceof ApiClientError
          ? serverErrors(error)
          : { form: 'Unable to update your profile. Please try again.' },
      )
    } finally {
  ...
  async function handlePasswordSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPasswordErrors(
        error instanceof ApiClientError
          ? serverErrors(error)
          : { form: 'Unable to change your password. Please try again.' },
      )
    } finally {
        ...
        <div className="settings-stack">
          <section className="settings-panel" aria-labelledby="settings-profile-heading">
            <form onSubmit={(event) => void handleProfileSubmit(event)} noValidate>
              ...
              {profileSuccess ? <SuccessBanner>{profileSuccess}</SuccessBanner> : null}
              {profileErrors.form ? (
              ...
              <div className="form-field">
                ...
                <input
                  ...
                  aria-describedby={
                    profileErrors.displayName ? 'account-display-name-error' : undefined
                  }
                  autoComplete="name"
                />
                {profileErrors.displayName ? (
                  <p id="account-display-name-error" className="field-error" role="alert">
                    {profileErrors.displayName}
                  </p>
                ) : null}
              </div>
          ...
          <section className="settings-panel" aria-labelledby="settings-password-heading">
            <form onSubmit={(event) => void handlePasswordSubmit(event)} noValidate>
              <h2 id="settings-password-heading">Change password</h2>
              {passwordSuccess ? <SuccessBanner>{passwordSuccess}</SuccessBanner> : null}
              {passwordErrors.form ? (
                <p className="form-error" role="alert">
                  {passwordErrors.form}
                </p>
                ...
                return (
                  <div className="form-field" key={field}>
                    ...
                    <input
                      ...
                      aria-invalid={passwordErrors[field] ? true : undefined}
                      aria-describedby={passwordErrors[field] ? `${id}-error` : undefined}
                      autoComplete={
          ...
          <section className="settings-panel" aria-labelledby="settings-session-heading">
            ...
            <p className="settings-panel__hint">Sign out of LedgerBloom on this device.</p>
            <button
              type="button"
